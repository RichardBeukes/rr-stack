...
namespace NzbDrone.Core.Parser
{
    public static class LanguageParser
    {
        ...
        // Unicode script detection for non-Latin release titles
        private static readonly Regex CyrillicRegex = new (@"[\u0400-\u04FF]{3,}", RegexOptions.Compiled);
        private static readonly Regex CjkRegex = new (@"[\u3000-\u9FFF\uAC00-\uD7AF]{2,}", RegexOptions.Compiled);
        private static readonly Regex ArabicRegex = new (@"[\u0600-\u06FF]{3,}", RegexOptions.Compiled);

        // Russian/Ukrainian dubbing and release group tags
        private static readonly Regex RussianDubbingRegex = new (@"\b(?:MVO|AVO|DVO|Dub\.(?:RUS|Ukr)|DUAL\.RUS|RuTracker|Generalfilm|D\.KOSHARA|HDRezka|nnmclub|kinozal|rutor|LostFilm|BaibaKo|NovaFilm|NewStudio|ColdFilm|Jaskier)\b", RegexOptions.Compiled | RegexOptions.IgnoreCase);
        ...
        public static List<Language> ParseLanguages(string title)
        {
            ...
            // Unicode script detection — catches releases with non-Latin titles
            // that would otherwise fall through as Unknown
            if (CyrillicRegex.IsMatch(title))
            {
                Logger.Trace("Detected Cyrillic script in release title, adding Russian");
                languages.Add(Language.Russian);
            }

            if (CjkRegex.IsMatch(title))
            {
                Logger.Trace("Detected CJK script in release title, adding Chinese");
                languages.Add(Language.Chinese);
            }

            if (ArabicRegex.IsMatch(title))
            {
                Logger.Trace("Detected Arabic script in release title, adding Arabic");
                languages.Add(Language.Arabic);
            }

            // Russian/Ukrainian dubbing tags and release groups
            if (RussianDubbingRegex.IsMatch(title))
            {
                Logger.Trace("Detected Russian dubbing/release group tag in title");
                languages.Add(Language.Russian);
            }
