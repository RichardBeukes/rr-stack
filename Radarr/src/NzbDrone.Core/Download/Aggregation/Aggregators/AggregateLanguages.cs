...
namespace NzbDrone.Core.Download.Aggregation.Aggregators
{
    public class AggregateLanguages : IAggregateRemoteMovie
    {
        ...
        public RemoteMovie Aggregate(RemoteMovie remoteMovie)
        {
            ...
            // When language can't be parsed from the release title, keep it as Unknown
            // rather than blindly assuming the movie's original language. The previous
            // behavior of falling back to OriginalLanguage caused non-English releases
            // (e.g., Russian dubs with clean English titles) to be misclassified as
            // English and bypass language filters.
            if (languages.Count == 0 || (languages.Count == 1 && languages.First() == Language.Unknown))
            {
                _logger.Debug("Language couldn't be parsed from release, keeping as Unknown (movie original: {0})", movie.MovieMetadata.Value.OriginalLanguage.Name);
            }
