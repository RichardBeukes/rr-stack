        ...
        where TSettings : IIndexerSettings, new()
    {
        ...
        protected virtual async Task<IndexerPageableQueryResult> FetchReleases(Func<IIndexerRequestGenerator, IndexerPageableRequestChain> pageableRequestChainSelector, SearchCriteriaBase searchCriteria, bool isRecent = false)
        {
            ...
            catch (WebException webException)
            {
                ...
                {
                    // Attempt mirror failover: if the indexer has alternative URLs,
                    // try the next one before recording failure.
                    var currentBase = Settings.BaseUrl?.TrimEnd('/');
                    var mirrorUrls = IndexerUrls.Where(u => !u.TrimEnd('/').Equals(currentBase, StringComparison.OrdinalIgnoreCase)).ToArray();

                    if (mirrorUrls.Length > 0)
                    {
                        var nextUrl = mirrorUrls[0];
                        _logger.Warn(webException, "{0} is unreachable ({1}), failing over to mirror: {2}", this, webException.Status, nextUrl);
        ...
        protected virtual async Task<ValidationFailure> TestConnection()
        {
            try
            {
                ...
                {
                    // If the indexer has proxy tags (e.g., FlareSolverr), empty results
                    // during validation may be transient. Allow save with a warning
                    // instead of blocking the add entirely.
                    if (Definition.Tags.Any())
                    {
                        _logger.Warn("Query returned no results, but indexer has proxy tags configured. Allowing save.");
                    }
                    else
                    {
            ...
            {
                // If a proxy (e.g., FlareSolverr) is configured via tags, CF protection
                // during validation is expected and should not block saving the indexer.
                // The proxy will handle it during actual searches.
                if (Definition.Tags.Any())
                {
                    _logger.Warn("CloudFlare protection detected during validation, but proxy is configured. Allowing save.");
                }
                else
                {
