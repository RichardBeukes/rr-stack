...
namespace NzbDrone.Core.IndexerSearch
{
    ...
    public class ReleaseSearchService : IReleaseSearchService
    {
        ...
        private async Task<IList<ReleaseInfo>> Dispatch(Func<IIndexer, Task<IndexerPageableQueryResult>> searchAction, SearchCriteriaBase criteriaBase)
        {
            ...
            // Limit concurrent indexer requests to prevent connection pool exhaustion
            // and excessive load when many apps search simultaneously.
            // 12 indexers * 50 parallel searches = 600 concurrent without this cap.
            var semaphore = new SemaphoreSlim(25);
            var tasks = indexers.Select(async x =>
            {
                await semaphore.WaitAsync();
                try
                {
                    return await DispatchIndexer(searchAction, x, criteriaBase);
                }
                finally
                {
                    semaphore.Release();
