        ...
        private ParsedTrackInfo ReadPdf(string file)
        {
            ...
            try
            {
                ...
                if (book.Info != null)
                {
                    ...
                    result.Publisher = book.Info.Creator.IsNotNullOrWhiteSpace()
                        ? book.Info.Creator
                        : book.Info.Producer;

                    // Extract ISBN from Subject or Keywords fields if present.
                    // Many publishers embed ISBN in these metadata fields.
                    var subjectAndKeywords = $"{book.Info.Subject} {book.Info.Keywords}";
                    if (subjectAndKeywords.IsNotNullOrWhiteSpace())
                    {
                        ...
                        {
                            result.Isbn = isbnMatch.Groups[1].Value;
                            _logger.Debug("Found ISBN {0} in PDF metadata", result.Isbn);
                        }
                    }
