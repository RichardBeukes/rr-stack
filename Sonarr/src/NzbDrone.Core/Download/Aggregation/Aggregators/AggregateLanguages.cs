...
namespace NzbDrone.Core.Download.Aggregation.Aggregators
{
    public class AggregateLanguages : IAggregateRemoteEpisode
    {
        ...
        public RemoteEpisode Aggregate(RemoteEpisode remoteEpisode)
        {
            ...
            // rather than blindly assuming the series' original language. The previous
            // behavior caused non-English releases with clean titles to be misclassified.
            if (languages.Count == 0 || (languages.Count == 1 && languages.First() == Language.Unknown))
            {
                _logger.Debug("Language couldn't be parsed from release, keeping as Unknown (series original: {0})", series.OriginalLanguage.Name);
            }
