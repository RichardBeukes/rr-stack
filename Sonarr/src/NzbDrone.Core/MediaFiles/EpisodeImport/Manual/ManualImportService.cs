    ...
    public class ManualImportService : IExecute<ManualImportCommand>, IManualImportService
    {
        ...
        private ManualImportItem ProcessFile(string rootFolder, string baseFolder, string file, string downloadId, Series series = null)
        {
            try
            {
                ...
                if (series == null)
                {
                    // Strip website prefixes (e.g., "www.UIndex.org - ") from folder
                    // names before attempting series lookup, as these break matching.
                    var folderName = relativeFile.Split('\\', '/')[0];
                    var cleanedFolder = Parser.ParserCommon.WebsitePrefixRegex.Replace(folderName);
                    series = _parsingService.GetSeries(cleanedFolder);
                }
