...
namespace NzbDrone.Core.MediaFiles.EpisodeImport.Specifications
{
    public class QualityVerificationSpecification : IImportDecisionEngineSpecification
    {
        ...
        public ImportSpecDecision IsSatisfiedBy(LocalEpisode localEpisode, DownloadClientItem downloadClientItem)
        {
            ...
            if (localEpisode.MediaInfo.Width == 0 || localEpisode.MediaInfo.Height == 0)
            {
                _logger.Debug("Resolution not available in MediaInfo, skipping check");
                return ImportSpecDecision.Accept();
            }

            var claimedQuality = localEpisode.Quality?.Quality;
            if (claimedQuality == null || claimedQuality == Quality.Unknown)
            {
                return ImportSpecDecision.Accept();
            }

            var actualWidth = localEpisode.MediaInfo.Width;
            var claimedResolution = claimedQuality.Resolution;

            // No resolution claim to verify (e.g., SDTV)
            ...
            if (actualWidth < minimumWidth)
            {
                ...
                return ImportSpecDecision.Reject(
                    ...
                    claimedQuality.Name,
                    actualWidth,
                    localEpisode.MediaInfo.Height,
                    minimumWidth,
                    resolutionLabel);
            }

            _logger.Debug("Resolution verified: claimed {0}, actual {1}x{2}", claimedQuality.Name, actualWidth, localEpisode.MediaInfo.Height);
            return ImportSpecDecision.Accept();
        }
    }
}
